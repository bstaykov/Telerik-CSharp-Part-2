...
class ExpressionCalculator
{
    ...
    static void Main()
    {
        ...
        Console.WriteLine(Evaluate(Parse(Tokenize("(3 - 1) * pow(pow(4 - 2, pow(ln(2.71), -1 + 4 + -3)), 10)"))));
    }

    private static double PowerOf(double number, double power)
    {
        if (power < -1)
        {
            power++;
            return (1 / number) * PowerOf(number, power);
        }
        else if (power == -1)
        {
            return 1 / number;
        ...
        {
            power--;
            return number * PowerOf(number, power);
        }
    }
}
